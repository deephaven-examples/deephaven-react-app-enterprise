...
export async function loadDashboard(
  ...
  const query = await loadQuery(client, undefined, "WebClientData");
  const table = await query.getTable("workspaceData");
  const idColumn = table.findColumn("Id");
  const dataColumn = table.findColumn("Data");
  table.applyFilter([
    idColumn.filter().eq(dh.FilterValue.ofString(dashboardId)),
  ]);
  table.setViewport(0, 0, [dataColumn]);
  const viewportData = await table.getViewportData();
  const dashboard = JSON.parse(viewportData.getData(0, dataColumn));
  return dashboard;
}
